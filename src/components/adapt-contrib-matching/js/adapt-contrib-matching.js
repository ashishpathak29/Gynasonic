    ...
    var Matching = QuestionView.extend({
        ...
        setScore: function() {
            ...
            if (this.model.get('_isCorrect')) {
                ...
                $('._fButtondisable .trickle-button-inner.locking button').css('display', 'inline-block');
				var page = this.getParentModel(this.model, "block");
				
				var storeClassess = page.get("_classes").split(" ")[0];
				
				if (storeClassess == "_showGotoNextButon") 
				{
					 $('.custom_go-next .go-next-component .go-next-button').css('display', 'inline-block');			
				}
                return;
            }
